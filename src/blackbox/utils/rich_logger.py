...
class RichLogger:
    ...
    def progress(self, disable: bool = True) -> Progress:
        """
        Return a Rich progress bar that is safe for concurrent or non-interactive use.
        Use `disable=True` to suppress rendering (e.g. inside multiprocessing).
        """
        if disable or os.getenv("DISABLE_PROGRESS") == "1":
            return Progress(disable=True, console=self.console, transient=True)
        return Progress(
